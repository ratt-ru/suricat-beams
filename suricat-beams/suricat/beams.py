...
class GMRT_BeamWizard(object):
    ...
    def __init__(self, image_name: str):
        ...
        log.info(f"image centre is at {self.centre}")
        # location could be made configurable        
        self.default_location = EarthLocation.of_site("GMRT") 
        log.info(f"location is GMRT ({self.default_location})")
    ...
    def ugmrt_beam_poly(self, freqs: float, sep: np.array):
        coeffs_b5 = [1.0, -2.614e-3, 27.594e-7, -13.268e-10,  2.395e-13]  # Band-5 coefficients
        coeffs_b4 = [1.0, -3.263e-3, 42.618e-7, -25.580e-10,  5.823e-13]  # Band-4 coefficients
        coeffs_b3 = [1.0, -3.129e-3, 38.816e-7, -21.608e-10,  4.483e-13]  # Band-3 coefficients
        coeffs_b2 = [1.0, -3.089e-3, 39.314e-7, -23.011e-10,  5.037e-13]  # Band-2 coefficients
        if freqs>=125 and freqs <=250:
            coeffs = coeffs_b2
        elif freqs>=250 and freqs<=500:
            coeffs = coeffs_b3
        elif freqs>=550 and freqs<=850:
            coeffs = coeffs_b4
        elif freqs>=1050 and freqs<=1450:
            coeffs = coeffs_b5
        else:
            raise RuntimeError("GMRT band could not be recognised")

        #sep = Angle(sep,unit='deg')
        
        freq_GHz = freqs*1e-3 # Expecting frequency input in MHz
        sep_arcmin = sep.arcmin
        r = sep_arcmin * freq_GHz

        pb = np.zeros_like(r)
        for i, c in enumerate(coeffs):
            pb += c * r**(2 * i)  #analytical 8th order polynomial for uGMRT
